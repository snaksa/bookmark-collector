...
class SingleLambdaHandler extends BaseHandler {
    ...
    async run(): Promise<Response> {
        ...
        const labelBookmarks = await this.labelRepository.findBookmarks(this.labelId);

        const bookmarks = labelBookmarks.map((labelBookmark) => new Bookmark(labelBookmark.bookmarkId, this.userId, labelBookmark.bookmarkUrl, labelBookmark.isFavorite, labelBookmark.isArchived));
